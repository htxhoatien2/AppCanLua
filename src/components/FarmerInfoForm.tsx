...
import { POPULAR_RICE_VARIETIES, HOA_TIEN_LOCATIONS, SAMPLE_TRUCKS, SAMPLE_OPERATORS } from '../data/riceData';
  ...
  return (
    <div className={`p-4 sm:p-5 rounded-3xl border shadow-xl transition-all duration-300 ${
      ...
      <div className="flex justify-between items-center mb-3 pb-2 border-b border-amber-500/20">
        ...
        >
          <span>👤 THÔNG TIN CHỦ RUỘNG, XE NHẬN & NGƯỜI CÂN (HTX HÒA TIẾN 2)</span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 text-sm pt-1">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full p-2.5 rounded-xl border font-bold focus:outline-none focus:ring-4 focus:ring-amber-500/30 focus:border-amber-500 transition-all ${
                darkMode ? 'bg-slate-950 border-slate-700 text-amber-300' : 'bg-amber-50/90 border-amber-300 text-amber-950'
              }`}
            />
            <datalist id="operators-list">
              {SAMPLE_OPERATORS.map((op, i) => <option key={i} value={op} />)}
            </datalist>
          </div>
          ...
          <div>
            ...
            <datalist id="trucks-list">
              {SAMPLE_TRUCKS.map((t, i) => <option key={i} value={t} />)}
            </datalist>
          </div>

          {/* Rice Variety (Giống Lúa Đà Nẵng / Miền Trung) */}
          <div>
            <label className="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1">
          ...
          {/* Advanced Tare Deduction Options (Trừ Bì Nâng Cao) */}
          <div>
            <div className="flex justify-between items-center mb-1">
              ...
              <select
                ...
                className="text-[10px] font-bold text-amber-700 bg-amber-50 dark:bg-slate-800 rounded px-1.5 py-0.5 border border-amber-300/60"
              >
            ...
            {sessionInfo.tareType === 'fixed_total' ? (
              <input
                ...
                className={`w-full p-2.5 rounded-xl border font-bold focus:outline-none focus:ring-4 focus:ring-amber-500/30 ${
          ...
          {/* Advanced Impurity / Moisture Deduction Options (Trừ Lép / Độ Ẩm Nâng Cao) */}
          <div>
            <div className="flex justify-between items-center mb-1">
              <label className="text-xs font-bold text-slate-600 dark:text-slate-300">
            ...
            {sessionInfo.impurityType === 'fixed_kg' ? (
              ...
              <div className="flex items-center gap-1">
                <input
                  ...
                  className={`w-full p-2.5 rounded-xl border font-bold text-xs focus:outline-none focus:ring-4 focus:ring-amber-500/30 ${
                    darkMode ? 'bg-slate-950 border-slate-700 text-blue-400' : 'bg-blue-50 border-blue-300 text-blue-900'
                  }`}
                />
          ...
          {/* Location / Field (Điểm Cân / Cánh Đồng HTX Hòa Tiến 2) */}
          <div className="col-span-2">
            ...
            <input
              ...
              className={`w-full p-2.5 rounded-xl border text-xs sm:text-sm font-semibold focus:outline-none focus:ring-4 focus:ring-amber-500/30 ${
                darkMode ? 'bg-slate-950 border-slate-700 text-white' : 'bg-slate-50 border-slate-300 text-slate-900'
              }`}
            />
            <datalist id="locations-list">
              {HOA_TIEN_LOCATIONS.map((loc, i) => <option key={i} value={loc} />)}
            </datalist>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className={`w-full p-2.5 rounded-xl border font-semibold focus:outline-none focus:ring-4 focus:ring-amber-500/30 ${
                darkMode ? 'bg-slate-950 border-slate-700 text-white' : 'bg-slate-50 border-slate-300 text-slate-900'
              }`}
            />
          </div>
