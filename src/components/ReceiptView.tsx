  ...
  return (
    <div className="space-y-4 max-w-2xl mx-auto pb-8">
      <div className="flex justify-between items-center print:hidden">
        <button
          onClick={onBack}
          className="flex items-center gap-1.5 text-xs font-lexend font-bold text-slate-700 dark:text-slate-200 bg-slate-200/80 dark:bg-slate-800 hover:bg-slate-300 px-4 py-2.5 rounded-2xl transition-all shadow-sm"
        >
          <span>← Quay lại</span>
        </button>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={onCopyZalo}
            className="flex items-center gap-1.5 text-xs font-lexend font-extrabold text-white bg-blue-600 hover:bg-blue-700 px-4 py-2.5 rounded-2xl shadow-lg transition-all border border-blue-400/30"
          >
            <span>📋 Copy Zalo</span>
          </button>
          <button
            onClick={handlePrint}
            className="flex items-center gap-1.5 text-xs font-lexend font-extrabold text-white bg-emerald-600 hover:bg-emerald-700 px-4.5 py-2.5 rounded-2xl shadow-lg transition-all border border-emerald-400/30"
          >
      ...
      <div
        ...
        className="bg-white text-slate-900 p-6 sm:p-9 rounded-3xl shadow-2xl border border-slate-200 font-sans space-y-6 relative overflow-hidden"
      >
        {/* Header Banner */}
        ...
        <div className="grid grid-cols-2 gap-3 text-xs bg-amber-50/90 p-4 rounded-2xl border border-amber-200/80">
          <div>
            ...
            <p className="font-lexend font-extrabold text-sm sm:text-base text-slate-900">{session.farmerName || '................................'}</p>
            {session.farmerPhone && <p className="text-[11px] text-slate-600 font-semibold">SĐT: {session.farmerPhone}</p>}
          </div>
        ...
        <div className="space-y-2 text-xs border-t border-b border-slate-200 py-4">
          <div className="flex justify-between py-1">
            <span className="text-slate-600">Tổng số bao lúa:</span>
            <span className="font-lexend font-extrabold text-sm">
          ...
          <div className="flex justify-between py-1 border-t border-slate-100">
            ...
            <span className="font-bold text-sm">{formatNumber(calc.grossWeight)} kg</span>
          </div>
          ...
          <div className="flex justify-between py-2.5 border-t-2 border-slate-900 text-base font-lexend font-black text-amber-900 bg-amber-50 px-3 rounded-xl">
            <span>CÂN RÒNG THỰC TẾ:</span>
            <span className="text-lg">{formatNumber(calc.finalNetWeight)} kg</span>
          </div>

          <div className="flex justify-between py-2.5 border-t border-slate-200 text-lg font-lexend font-black text-slate-900">
            <span>TỔNG THÀNH TIỀN:</span>
            <span className="text-emerald-700 text-xl">{formatVND(calc.totalAmount)}</span>
          </div>
          ...
          {calc.depositAmount > 0 && (
            <>
              ...
              <div className="flex justify-between py-2.5 border-t-2 border-rose-500 text-base font-lexend font-black text-rose-700 bg-rose-50 px-3 rounded-xl">
                <span>CÒN LẠI THANH TOÁN:</span>
                <span className="text-lg">{formatVND(calc.remainingPayable)}</span>
              </div>
            </>
          )}
        </div>
        ...
        <div className="space-y-2">
          <div className="font-lexend font-bold text-xs uppercase text-slate-700 flex justify-between items-center">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5 text-[11px] font-mono border p-2.5 rounded-2xl bg-slate-50 max-h-52 overflow-y-auto">
            {entries.map((e, idx) => (
              <div key={idx} className="p-1.5 border rounded-xl bg-white text-center shadow-xs">
                <span className="text-[9px] text-slate-400 block font-sans">#{idx + 1}</span>
                <strong className="text-slate-900 text-xs">{e.weight} kg</strong>
                {e.bagsCount > 1 && (
                  <span className="text-[9px] block text-amber-700 font-bold font-sans">({e.bagsCount} bao)</span>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="pt-8 grid grid-cols-2 text-center text-xs text-slate-600 gap-4">
          <div>
            <p className="font-lexend font-bold text-slate-900 uppercase">CHỦ RUỘNG (BÊN BÁN)</p>
            <p className="text-[10px] italic text-slate-400 mt-0.5">(Ký và ghi rõ họ tên)</p>
            <div className="h-20"></div>
            <p className="font-bold text-slate-800">{session.farmerName || '....................'}</p>
          </div>
